...
git clone https://github.com/formio/deployment-examples.git
cd deployment-examples/docker/quickstart
...
When you see `🚀 Form.io is now fully up and running!`, open **http://localhost:3000** and sign in with the `ADMIN_EMAIL` and `ADMIN_PASS` you set in `.env`.
...
|---|---|---|
| `formio-proxy` | `caddy:2-alpine` | TLS termination, path-based routing |
| `formio-api` | `formio/formio-enterprise:9.7.4` | Core API and Developer Portal |
| `formio-pdf` | `formio/pdf-server:5.13.1` | PDF generation, form rendering to PDF |
...
|---|---|---|
| Container exits with `Contact sales for a valid license key.` | `LICENSE_KEY` is blank in `.env` | Paste your key and restart. The compose file uses `${LICENSE_KEY:?...}` to fail fast. |
...
| File uploads fail with a CORS error | SeaweedFS has no CORS policy for the browser origin | Configure CORS on the SeaweedFS bucket. |
...
- **Enable TLS.** The Caddyfile listens on `:80`. Replace it with a real hostname to get automatic Let's Encrypt certificates, or terminate TLS at an external load balancer.
- **Do not expose database or S3 ports publicly.** Only the proxy port should be reachable from outside your network.
- **Pin all image tags.** Avoid `:latest` in any environment.
...
| Channel | When to use |
|---|---|
| 🐛 [GitHub Issues](https://github.com/formio/deployment-examples/issues) | Bugs or suggestions for these examples |
